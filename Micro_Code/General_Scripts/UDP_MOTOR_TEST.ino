...
void loop() {
  ...
  if (packetSize)
  {
    ...
    if((message.length() == 19) && (message.charAt(0) =='d')){
    for(int i = 0; i <= 5;i++){
...
// a simple function to check for a specific connection (we will define the ip's later)
//if the connection does not exist then it writes 1500 ie 0 to all motors. The ip's to be checked will be
//the basestation ip addresses
// will kill if ethernet cable disconnected
void watchDog(){
if(Udp.linkStatus()==LinkOFF){
  LeftMotors[0].writeMicroseconds(MotorRelaxVal);
  LeftMotors[1].writeMicroseconds(MotorRelaxVal);
  LeftMotors[2].writeMicroseconds(MotorRelaxVal);
  RightMotors[0].writeMicroseconds(MotorRelaxVal);
  RightMotors[1].writeMicroseconds(MotorRelaxVal);
  RightMotors[2].writeMicroseconds(MotorRelaxVal);
}
